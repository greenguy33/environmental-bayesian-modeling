...
for index, row in enumerate(data.itertuples()):
    if row.iso in countries_to_remove:
     ...
     # 1960 data removed because there is no Delta GDP for first year which causes problem in 1960 year effect
    elif row.year == 1960:
        indices_to_drop.append(index)
...
with open ('models/burke-reproduction-mcmc-fixed-effects-grad-effects-missing-rows-imputed-country-specific-priors.pkl', 'wb') as buff:
	pkl.dump({
		"prior":prior,
		"trace":trace,
		"posterior":posterior,
