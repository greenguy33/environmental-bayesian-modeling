...
with pm.Model() as model:
    ...
    precip_country_coefs = pt.expand_dims(pm.Normal("precip_country_coefs", 0, 5, shape=(len(set(data.iso))-1)),axis=1)
    ...
    precip_prior = pm.Deterministic(
        "precip_prior", 
        precip_intercept + 
        (temp_scaled * temp_precip_coef) +
        (pt.sqr(temp_scaled) * temp_sq_precip_coef) +
...
with open ('models/nature_reproduction/burke-reproduction-mcmc-fixed-effects-grad-effects-precip-dep.pkl', 'wb') as buff:
	pkl.dump({
		"prior":prior,
		"trace":trace,
		"posterior":posterior,
