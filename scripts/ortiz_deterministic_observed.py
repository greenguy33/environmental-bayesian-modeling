...
data = pd.read_csv("data/ortiz-bobea-dataset.csv")
...
with pm.Model() as model:
    ...
    tfp_intercept = pm.Normal('tfp_intercept',-.3,1)
    fd_temp_tfp_coef = pm.Normal('fd_temp_tfp_coef',0,.005)
    fd_sq_temp_tfp_coef = pm.Normal('fd_sq_temp_tfp_coef',0,.0001)
    fd_precip_tfp_coef = pm.Normal("fd_precip_tfp_coef",0,.0001)
    fd_sq_precip_tfp_coef = pm.Normal("fd_sq_precip_tfp_coef",0,.0001)
...
with open ('models/ortiz-bobea-reproduction-year-country-fixed-effects-deterministic-observed.pkl', 'wb') as buff:
    pkl.dump({
        "prior":prior,
        "trace":trace,
        "posterior":posterior
